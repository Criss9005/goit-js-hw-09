...
function createPromise(position, delay) {
    ...
    setTimeout(()=>{
        if (shouldResolve) (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay}ms`);
        else (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay}ms`);
    }, delay);
}
...
form.addEventListener("submit", (e)=>{
    ...
    for(let i = 0; i <= amount.value; i++)if (i == 0) createPromise(i + 1, stepInt);
    else {
